...
// "Database" es el nombre de referencia que se le dara a los logs que provengan de esta funcion
  ...
  const connect = () => {
    mongoose
      ...
      .then(() => {
        ...
        log.info('Connected to Database successfully'); //los logger de tipo "info" son para entregar mensajes de anuncios
      })
      .catch(error => {
        // siempre debe ir un mensaje para hacer saber que ocurrio un error con el arranque del db
        log.error('Error connecting to database', error); // los logger de tipo "error" son para mostrar errores , alertas, warning
